...
[Check out the `add-rbac` branch](https://github.com/auth0-developer-hub/api_lumen_php_hello-world/tree/add-rbac) to see authorization and Role-Based Access Control (RBAC) in action using Auth0.
...
With the `.env` configuration values set, run the API server by issuing the following command from the root folder of the project:
